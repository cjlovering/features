...
@plac.opt(
    "prop", "prop to use", choices=["gap_forced-lexical"],
)
@plac.opt(
    "splitcount", "number of examples in train / test",
)
def main(prop="gap_forced-lexical", splitcount=1000, rates=[0, 0.001, 0.01, 0.1]):
    ...
    for (
        ...
        for _ in range(count):
            parts, info = counter_template(
                2, parenthetical_probability=0, prefix_subj=counter_subj
            )
            sent = stringify(parts)
